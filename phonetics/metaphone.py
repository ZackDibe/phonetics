...
def dmetaphone(source):
    ...
    start = {'KN', 'GN', 'PN', 'PS', 'WR'}
    if source[:2] in start:
        index += 1
    elif source.get(0) == 'X':
        index += 1
        primary.append('S')
    ...
    while index < len(source):
        func = locals().get('process_{}'.format(source[index]))
        if func:
            result = func(source, index)
            result[0] and primary.extend(result[0])
            result[1] and secondary.extend(result[1])
            index += result[2]
        else:
            index += 1
